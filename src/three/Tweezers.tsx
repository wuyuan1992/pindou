...
const ROD_TIP_Y = -0.225;
const ROD_OFFSET_X = 0.21;
const ROD_HANDLE_Y = STACK_SIZE / 2 + 1.5;
const ROD_LENGTH = ROD_HANDLE_Y - ROD_TIP_Y;
const ROD_CENTER_Y = (ROD_TIP_Y + ROD_HANDLE_Y) / 2;

const HANDLE_THICKNESS = 0.1;
const HANDLE_WIDTH = ROD_OFFSET_X + 0.11;
...
export function Tweezers() {
  ...
  return (
    <group>
      ...
      {/* 手柄连接块（y=手柄端） */}
      <mesh
        position={[ROD_OFFSET_X / 2, ROD_HANDLE_Y + 0.025, 0]}
        material={handleMat}
        castShadow
      >
        <boxGeometry args={[HANDLE_WIDTH, 0.175, HANDLE_THICKNESS]} />
      </mesh>

      {/* 镊尖收尖 */}
      <mesh position={[ROD_OFFSET_X * 0.5, ROD_TIP_Y - 0.02, 0]} material={metal}>
        <cylinderGeometry args={[ROD_RADIUS * 0.4, ROD_RADIUS * 1.2, 0.06, 16]} />
      </mesh>
    </group>
  );
}
